...
by repeating frames with imageio (MVP; prompt is reserved for future AI steps).
"""
...
def generate_video(image, prompt: str, duration_sec: float) -> tuple[str | None, str]:
    """
    Build a video by repeating the uploaded image for the requested duration.
...
def main() -> None:
    with gr.Blocks(title="VideoKas AI") as demo:
        gr.Markdown("# VideoKas AI\nTurn an image into a short video (MVP demo).")
